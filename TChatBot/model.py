...
class ChatBotDNN:
    ...
    def trainModel(self,x,y,epochs = 50,test_size = 0.2,batch_size = 10):
        disp = PrettyTable(['Training TChatBot DNN model.....'])
        ...
        path_to_model = saved_model_path / model_name / model_architecture[0]
        ...
        try:  
            ...
            print("-----+-----+-----+-----+-----+-----+-----+------+------+-----+------+------")
            print("Model saved in disc as \'saved_model.pb\' file in path: {}".format("saved_model/{}".format(model_name)+"/"+model_architecture[0]+"/"+model_version))
            print("-----+-----+-----+-----+-----+-----+-----+------+------+-----+------+------\n")  
...
class ChatBotCNN:
    ...
    def trainModel(self,x,y,epochs = 50,test_size = 0.2,batch_size = 10):
        disp = PrettyTable(['Training TChatBot CNN model.....'])

        #Cross-validation
        x_train ,x_test , y_train, y_test = model_selection.train_test_split(x,y, test_size = test_size)

        #Resizing X values for CNN architecture
        x_train = x_train.reshape(len(x_train),1,len(x_train[0])).astype('float64')
        x_test = x_test.reshape(len(x_test),1,len(x_test[0])).astype('float64')
        ...
        path_to_model = saved_model_path / model_name / model_architecture[1]
        path = os.path.join(path_to_model,model_version) 
        ...
        try:  
            ...
            print("                         Saving trained Model version {}......".format(model_version))
            print("-----+-----+-----+-----+-----+-----+-----+------+------+-----+------+------")
            print("Model saved in disc as \'saved_model.pb\' file in path: {}".format("saved_model/{}".format(model_name)+"/"+model_architecture[1]+"/"+model_version))
            print("-----+-----+-----+-----+-----+-----+-----+------+------+-----+------+------\n")  
        except OSError as error:  
            print("Path already exists") 
    ...
    def validateModel(self,x,y):
        ...
        #Reshape 
        x = x.reshape(len(x),1,len(x[0])).astype('float64')
        self.classes = sorted(getClasses())
    ...
    def chat(self,words,data):
        ...
        while True:
            ...
            if(inp.lower()=='quit'):
                ...
                break
            x = bagOfWords(inp,words)
            x = x.reshape(len(x),1,len(x[0])).astype('float64')
            
            chatOut = self.model.predict([x])[0]
            chatOut_index = np.argmax(chatOut)
            tag = sort_classes[chatOut_index]
            intents = data["intents"]
            # confidence = chatOut[chatOut_index]
